  ...
  template = ` <button class='chat-btn'><img src = "./icons/comment.png" class = "icon" ></button>
    ...
    <div class='chat-popup'>
		...
		<div class='chat-header'>
			...
			<h3 class='bot-title'>Rasa Bot</h3>
...
function setBotResponse(val) {
  setTimeout(function () {
    if (val.hasOwnProperty("intent")) {
      //if we get parse api response from Rasa
      var response_intent =
        "intent: " + val.intent.name + ", confidence: " + val.intent.confidence;
      var response_intent_ranking =
        "intent_ranking: " +
        val.intent_ranking[0].name +
        ", confidence: " +
        val.intent_ranking[0].confidence +
        val.intent_ranking[1].name +
        ", confidence: " +
        val.intent_ranking[1].confidence +
        val.intent_ranking[2].name +
        ", confidence: " +
        val.intent_ranking[2].confidence;

      var BotResponse = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'>${response_intent}</span></div>`;
      var BotResponse2 = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'>${response_intent_ranking}</span></div>`;
      $(BotResponse).appendTo(".chat-area").hide().fadeIn(1000);
      $(BotResponse2).appendTo(".chat-area").hide().fadeIn(1000);
      scrollToBottomOfResults();
      chatInput.disabled = false;
      chatInput.focus();
    } else {
